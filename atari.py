...
class Atari:
    ...
    def __init__(self):
        game_name, game_mode, model_path, render, total_step_limit, total_run_limit, clip = self._args()
        env_name = game_name + "Deterministic-v4"  # Handles frame skipping (4) at every iteration
        env = MainGymWrapper.wrap(gym.make(env_name))
        self._main_loop(self._game_model(game_mode, game_name, env.action_space.n, model_path), env, render, total_step_limit, total_run_limit, clip)
    ...
    def _args(self):
        ...
        parser.add_argument("-m", "--mode", help="Choose from available modes: ddqn_training, ddqn_testing, ge_training, ge_testing. Default is 'ddqn_training'.", default="ddqn_training")
        ...
        parser.add_argument("-l", "--model", help="Choose the model path to use.", default=None)
        ...
        if str(model_path) is not None:
            print("Model name: " + str(model_path))
        ...
        return game_name, game_mode, model_path, render, total_step_limit, total_run_limit, clip

    def _game_model(self, game_mode,game_name, action_space, model_path):
        if game_mode == "ddqn_training":
            return DDQNTrainer(game_name, INPUT_SHAPE, action_space, model_path)
        elif game_mode == "ddqn_testing":
            return DDQNSolver(game_name, INPUT_SHAPE, action_space, model_path)
        elif game_mode == "ge_training":
            return GETrainer(game_name, INPUT_SHAPE, action_space, model_path)
        elif game_mode == "ge_testing":
            return GESolver(game_name, INPUT_SHAPE, action_space, model_path)
        else:
